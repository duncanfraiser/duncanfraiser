@extends('_layout.template')
@section('content')
<div class="container-fluid" style="padding: 0">

  {{-- color desk img spacer --}}
  <div class="col-md-12" style="padding: 0">
    <div class="mydeskColor">
    </div>
  </div>








{{-- project section section --}}
  <div class="col-md-10 col-md-offset-1 project">
    <div class="col-md-12 os-animation" data-os-animation="fadeIn" data-os-animation-delay=".3s">
      <div class="col-md-8">
        <h2><center><a target="blank" href="http://kennykens.com/">Kenny Kens & The Brown Bottle Boys</a></center></h2>
    		<p>Bacon ipsum dolor amet burgdoggen turkey ham hock salami bresaola. Short loin bacon fatback burgdoggen beef ribs shank chuck turducken flank. Fatback flank bresaola landjaeger leberkas. Short loin cupim tenderloin, pancetta shoulder beef ribs strip steak ham hock corned beef turkey kevin sirloin. Venison prosciutto ball tip landjaeger swine spare ribs jerky short ribs bacon.</p>
      </div>
      <div class="col-md-4">
    	<a target="blank" href="{{url('/img/kk-index.png')}}"><img style="width: 100%" src="{{url('/img/kk-index.png')}}"></a>
      </div>
    </div>
  </div>










  {{-- black and white desk img spacer --}}
  <div class="col-md-12" style="padding: 0">
    <div class="mydeskBW">
    </div>
  </div>




{{-- project section section --}}
  <div class="col-md-10 col-md-offset-1 project">
    <div class="col-md-12 os-animation" data-os-animation="fadeIn" data-os-animation-delay=".3s">
      <div class="col-md-4">
    	<a target="blank" href="{{url('/img/kk-bio.png')}}"><img style="width: 100%" src="{{url('/img/kk-bio.png')}}"></a>
      </div>
      <div class="col-md-8">
    	<h2><center>Section Title</center></h2>
    		<p>Bacon ipsum dolor amet burgdoggen turkey ham hock salami bresaola. Short loin bacon fatback burgdoggen beef ribs shank chuck turducken flank. Fatback flank bresaola landjaeger leberkas. Short loin cupim tenderloin, pancetta shoulder beef ribs strip steak ham hock corned beef turkey kevin sirloin. Venison prosciutto ball tip landjaeger swine spare ribs jerky short ribs bacon.</p>
      </div>
    </div>
  </div>



  {{-- color desk img spacer --}}
  <div class="col-md-12" style="padding: 0">
    <div class="mydeskColor">
    </div>
  </div>



{{-- project section section --}}
  <div class="col-md-10 col-md-offset-1 project">
    <div class="col-md-12 os-animation" data-os-animation="fadeIn" data-os-animation-delay=".3s">
      <div class="col-md-8">
    	<h2><center>Section Title</center></h2>
    		<p>Bacon ipsum dolor amet burgdoggen turkey ham hock salami bresaola. Short loin bacon fatback burgdoggen beef ribs shank chuck turducken flank. Fatback flank bresaola landjaeger leberkas. Short loin cupim tenderloin, pancetta shoulder beef ribs strip steak ham hock corned beef turkey kevin sirloin. Venison prosciutto ball tip landjaeger swine spare ribs jerky short ribs bacon.</p>
      </div>
      <div class="col-md-4">
    	<a target="blank" href="{{url('/img/kk-video.png')}}"><img style="width: 100%" src="{{url('/img/kk-video.png')}}"></a>
      </div>
    </div>
  </div>






  {{-- black and white desk img spacer --}}
  <div class="col-md-12" style="padding: 0">
    <div class="mydeskBW">
    </div>
  </div>





{{-- project section section --}}
  <div class="col-md-10 col-md-offset-1 project">
    <div class="col-md-12 os-animation" data-os-animation="fadeIn" data-os-animation-delay=".3s">
      <div class="col-md-4">
    	<a target="blank" href="{{url('/img/kk-booking.png')}}"><img style="width: 100%" src="{{url('/img/kk-booking.png')}}"></a>
      </div>
      <div class="col-md-8">
    	<h2><center>Section Title</center></h2>
    		<p>Bacon ipsum dolor amet burgdoggen turkey ham hock salami bresaola. Short loin bacon fatback burgdoggen beef ribs shank chuck turducken flank. Fatback flank bresaola landjaeger leberkas. Short loin cupim tenderloin, pancetta shoulder beef ribs strip steak ham hock corned beef turkey kevin sirloin. Venison prosciutto ball tip landjaeger swine spare ribs jerky short ribs bacon.</p>
      </div>
    </div>
  </div>




    {{-- color desk img spacer --}}
  <div class="col-md-12" style="padding: 0">
    <div class="mydeskColor">
    </div>
  </div>


{{-- project section section --}}
  <div class="col-md-10 col-md-offset-1 project">
    <div class="col-md-12 os-animation" data-os-animation="fadeIn" data-os-animation-delay=".3s">
      <div class="col-md-8">
      <h2><center>Section Title</center></h2>
        <p>Bacon ipsum dolor amet burgdoggen turkey ham hock salami bresaola. Short loin bacon fatback burgdoggen beef ribs shank chuck turducken flank. Fatback flank bresaola landjaeger leberkas. Short loin cupim tenderloin, pancetta shoulder beef ribs strip steak ham hock corned beef turkey kevin sirloin. Venison prosciutto ball tip landjaeger swine spare ribs jerky short ribs bacon.</p>
      </div>
      <div class="col-md-4">
      <a target="blank" href="{{url('/img/kk-merch.png')}}"><img style="width: 100%" src="{{url('/img/kk-merch.png')}}"></a>
      </div>
    </div>
  </div>











  {{-- black and white desk img spacer --}}
  <div class="col-md-12" style="padding: 0">
    <div class="mydeskBW">
    </div>
  </div>




</div>
@endsection  